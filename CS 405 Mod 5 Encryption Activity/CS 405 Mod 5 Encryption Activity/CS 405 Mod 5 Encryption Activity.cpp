...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    // source is the encrypted output
    std::string output = source;

    // loop through the source string char by char
    for (size_t i = 0; i < source_length; ++i)
    {
      // transformed each character based on an xor of the key modded constrained to key length using a mod
...
std::string read_file(const std::string& filename)
{
    ...
    // implemented loading the file text into a string
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    if (outputFile.is_open()) {
        // writing student name to the file
